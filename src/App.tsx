...
const navLinks = [
  { label: 'Why Linera', href: '#story' },
  { label: 'Telemetry', href: '#live' },
  { label: 'Creator Vaults', href: '#creators' },
...
const fallbackHeroMetrics = [
  { label: 'Average viewer payout / session', value: '0.86 $ATTN', accent: 'text-emerald-300' },
  { label: 'Creator payout speed', value: '2.3s', accent: 'text-sky-300' },
  { label: 'AI optimizations / hour', value: '1,200+', accent: 'text-rose-300' },
...
const brandOSPillars = [
  {
    ...
    points: ['Auto-generates ad variants', 'Personalizes per creator surface', 'Mints AdNFT receipts'],
  },
  {
    title: 'Live Budget Brain',
    points: ['Bids per impression in sub-seconds', 'Shifts funds to best-performing lanes', 'Halts waste automatically'],
  },
  {
    title: 'Truthful Analytics',
    points: ['Every impression notarized on-chain', 'Heatmaps show viewer, creator, spend', 'Anti-fraud + refund guardrails'],
  },
];
...
const attentionFinance = [
  {
    ...
    detail: 'Consistency, authenticity, and diverse creators raise your score above 700.',
  },
  {
    title: 'Unlock A-Fi Credit',
    detail: 'Score auto-qualifies you for 5 / 10 / 25 $ATTN micro-loans instantly.',
  },
  {
...
const experienceTracks = [
  {
    title: 'Viewer Wallet',
    bullets: ['Claim instantly verified $ATTN', 'Stack streak multipliers', 'Convert attention into micro-loans'],
  },
  {
    title: 'Creator Vaults',
    bullets: ['Accept or reject campaigns in real time', 'Stake idle revenue for boosted reach', 'Automated fraud scrubbing'],
  },
  {
    title: 'Advertiser Brand OS',
    bullets: ['Deploy AI co-pilot for bidding', 'Segment by microchains not cookies', 'Trace every micro-payment'],
  },
];
...
const developerToolkit = [
  {
    ...
    description: 'Pre-built hooks for wallet connect, GraphQL subscriptions, and microchain events.',
  },
  {
    title: 'CLI recipes',
    description: 'One-command scripts to mint demo wallets, seed viewers, and replay workloads.',
  },
  {
    title: 'Telemetry APIs',
    description: 'Webhooks for viewer attention streaks, advertiser pacing, and AI agent status.',
  },
];

const timelineMilestones = [
  { date: 'Wave 2', title: 'Attention Finance live on Testnet', detail: 'Viewers collateralize verified watch history for instant micro-loans.' },
  { date: 'Wave 3', title: 'Creator Vaults launch', detail: 'Programmable revenue splits, AI brand filters, vault staking, and premium drops.' },
  { date: 'ETH Denver', title: 'Brand OS beta', detail: 'Autonomous marketing agents escalate budgets cross-microchain with full auditability.' },
];

const supportChannels = [
  { label: 'Discord', detail: 'WaveHack builder lounge for live support.' },
  { label: 'Docs', detail: 'Linera microchain quick-start, GraphQL schema, SDK recipes.' },
  { label: 'Mentorship', detail: 'Weekly office hours with Linera core + AdLoom product team.' },
];

const fallbackEventsTable = [
  { viewerId: '@fluxseer', creatorId: 'PrimeLabs', advertiserId: 'PulseDrip', attnUnits: 6, reward: '18.0 $ATTN', latency: '2.3s' },
  { viewerId: '@zenwatcher', creatorId: 'NovaPod', advertiserId: 'EcoModa', attnUnits: 4, reward: '11.2 $ATTN', latency: '2.0s' },
  { viewerId: '@signalchaser', creatorId: 'SynthForge', advertiserId: 'HyperDrop', attnUnits: 5, reward: '15.5 $ATTN', latency: '1.8s' },
];
...
function App() {
  const { data: pulse, error: pulseError, loading: pulseLoading, endpoint } = useAdloomData();
  ...
  const realtimeMoments = useMemo(() => {
    ...
    return fallbackRealtimeMoments;
  }, [pulse]);
  ...
  const globalPulseStats = useMemo(() => {
    ...
    return [
      { label: 'Total impressions', value: formatCompactNumber(pulse.global.totalImpressions) },
      { label: 'Outstanding A-Fi credit', value: formatTokenValue(pulse.global.outstandingCredit) },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="relative overflow-hidden">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,#1d4ed8_0%,transparent_55%)] opacity-40" />
          ...
          <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/50">Linera WaveHack Special</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button className="hidden rounded-full border border-white/20 px-4 py-2 text-xs font-semibold text-white/80 transition hover:border-white hover:text-white md:inline-flex">
                Book Strategy Call
              </button>
              <button className="rounded-full bg-gradient-to-r from-emerald-400 to-sky-400 px-4 py-2 text-xs font-semibold text-slate-900">
                Launch App
              </button>
            </div>
          </div>
        ...
        <main className="relative z-10 mx-auto flex max-w-6xl flex-col gap-20 px-6 py-16 md:py-24">
          <section className="text-center">
            ...
            <p className="mx-auto mt-6 max-w-3xl text-lg text-white/70">
              Imagine YouTube, TikTok, Google Ads, AdSense, DeFi, and AI agents fused into one living interface. Every view,
              creation, bid, and integration pays out instantly. That is AdLoom X Ultra — the first platform where attention,
              creativity, and ad budgets operate in sync on-chain.
            </p>
            {liveStatus && (
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {heroMetrics.map((metric) => (
                <div key={metric.label} className="rounded-2xl border border-white/10 bg-white/5 p-6 text-left">
              ...
              {personaStories.map((persona) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-white/70 text-sm uppercase tracking-wide">User POV</p>
          ...
          <section className="rounded-3xl border border-white/10 bg-white/5 p-8">
            <div className="grid gap-8 md:grid-cols-3">
              {experienceTracks.map((track) => (
                <article key={track.title} className="flex flex-col rounded-2xl border border-white/10 bg-black/40 p-6">
                  <div className="text-xs uppercase tracking-wide text-white/60">{track.title}</div>
          ...
          <section className="rounded-3xl border border-white/10 bg-white/5 p-8 md:p-12">
            <div className="grid gap-10 md:grid-cols-[1.1fr_0.9fr]">
              <div>
                <p className="text-sm uppercase tracking-wide text-emerald-300">Viewer Journey</p>
                <h2 className="mt-3 text-3xl font-semibold text-white">“I watch, I earn, instantly.”</h2>
                <p className="mt-4 text-white/70">
                  Ads feel native — mini cards, swipeable spots, micro games — never intrusive. Each verified interaction lights up
                  your wallet feed with sub-second microchain receipts.
                </p>
                <div className="mt-8 rounded-2xl border border-white/10 bg-black/40 p-6">
                  ...
                  <div className="mt-4 space-y-3">
                    {['Eco Futures · +0.20 $ATTN', 'Indie Game Drop · +0.09 $ATTN', 'Creator Boost · +0.12 $ATTN'].map(
                      (entry) => (
                        <div key={entry} className="rounded-xl bg-white/5 px-4 py-3 text-sm text-white/80">
                          {entry}
                        </div>
                      ),
                    )}
                  ...
                  <div className="mt-5 flex items-center justify-between">
                    <div>
                      <p className="text-xs uppercase text-white/50">Attention Score</p>
                      <p className="text-2xl font-semibold text-emerald-300">742 Prime</p>
                    </div>
              ...
              <div className="space-y-6">
                <div className="rounded-2xl border border-white/10 bg-black/30 p-6">
                  <p className="text-sm uppercase tracking-wide text-sky-300">Leaderboard snapshot</p>
                ...
                <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-emerald-500/10 to-sky-500/10 p-6">
                  <p className="text-sm uppercase tracking-wide text-white/70">Attention Score unlocks</p>
                  <ul className="mt-4 space-y-3 text-sm text-white/80">
          ...
          <section id="live" className="grid gap-8 rounded-3xl border border-white/10 bg-black/40 p-8 md:grid-cols-[1.1fr_0.9fr]">
            <div>
              <p className="text-sm uppercase tracking-wide text-rose-300">Live Telemetry</p>
              <h2 className="mt-2 text-3xl font-semibold text-white">Attention engine dashboard</h2>
              <p className="mt-3 text-white/70">
                Every card below is live data from the Linera service. Swap endpoints, replay attention flows, and watch the AI agent pivot budgets
                in milliseconds.
              </p>
              <ul className="mt-6 space-y-3 text-sm text-white/80">
                {globalPulseStats.map((stat) => (
                  <li key={stat.label} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="rounded-2xl border border-white/10 bg-slate-950/70">
              ...
              <div className="divide-y divide-white/5 px-4 text-xs text-white/70">
                ...
                {liveEventsTable.map((event) => (
                  <div key={`${event.viewerId}-${event.creatorId}-${event.advertiserId}`} className="grid grid-cols-5 gap-3 px-2 py-3 text-sm">
            ...
            <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-indigo-900/50 to-black/50 p-8 md:p-12">
              <div className="md:flex md:items-start md:justify-between">
                <div className="max-w-2xl">
                  <p className="text-sm uppercase tracking-wide text-rose-300">Advertiser Operating System</p>
                  <h2 className="mt-3 text-3xl font-semibold text-white">BrandOS runs campaigns in real time</h2>
                  <p className="mt-4 text-white/70">
                    Toggle “AI Mode” and your agent handles creatives, bidding, allocations, verifications, and even refunds.
                    You watch the operating system evolve ads every 1–3 minutes based on what users respond to.
                  </p>
                </div>
              ...
              <div className="mt-10 grid gap-6 md:grid-cols-3">
                {brandOSPillars.map((pillar) => (
                  <div key={pillar.title} className="rounded-2xl border border-white/10 bg-black/30 p-6">
                    <p className="text-sm uppercase tracking-wide text-white/60">{pillar.title}</p>
              ...
              <div className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-6 text-sm text-white/70">
                {realtimeMoments.map((moment) => (
                  <div key={moment} className="border-b border-white/5 py-3 last:border-b-0">
                    {moment}
                  </div>
                ))}
              </div>
          ...
          <section id="creators" className="grid gap-8 md:grid-cols-2">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 md:p-8">
              <p className="text-sm uppercase tracking-wide text-amber-300">Developer POV</p>
              <h2 className="mt-3 text-3xl font-semibold text-white">“I integrate once, revenue streams forever.”</h2>
              <p className="mt-4 text-white/70">
                Whether it’s a game, browser extension, or social experiment, AdLoom’s SDK wraps your UX with transparent monetization.
                The microchain logs every impression, giving you verifiable data and instant payouts.
              </p>
              <div className="mt-6 space-y-4 text-sm text-white/80">
                {['SDK install · 4 lines of code', 'Revenue share auto-splits with creators + viewers', 'Marketplace lets advertisers choose your app', 'Telemetry stays private yet auditable'].map(
                  (item) => (
            ...
            <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-emerald-500/10 via-cyan-500/10 to-rose-500/10 p-6 md:p-8">
              <p className="text-sm uppercase tracking-wide text-emerald-200">Attention Finance (A-Fi)</p>
              <h2 className="mt-3 text-3xl font-semibold text-white">Turn loyalty into credit</h2>
              <p className="mt-4 text-white/70">
                Power users unlock micro-loans purely from their Attention Score. No bank. No paperwork. Just provable on-chain history of genuine engagement.
              </p>
          ...
          <section id="developers" className="rounded-3xl border border-white/10 bg-white/5 p-8">
            <div className="md:flex md:items-start md:justify-between">
              <div className="max-w-2xl">
                <p className="text-sm uppercase tracking-wide text-sky-300">Developer toolkit</p>
                <h2 className="mt-3 text-3xl font-semibold text-white">Build fully on-chain experiences without the drag</h2>
                <p className="mt-4 text-white/70">
                  Use the Linera SDK, GraphQL subscriptions, and our React hooks to mirror microchain state across desktop, mobile, and
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              {developerToolkit.map((item) => (
                <article key={item.title} className="rounded-2xl border border-white/10 bg-black/40 p-5">
          ...
          <section className="rounded-3xl border border-white/10 bg-black/40 p-8">
            <p className="text-sm uppercase tracking-wide text-white/60">Live timeline</p>
            <h2 className="mt-3 text-3xl font-semibold text-white">Road to ETH Denver demo day</h2>
            <div className="mt-8 space-y-6">
              {timelineMilestones.map((milestone) => (
                <div key={milestone.title} className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  <div className="text-xs uppercase tracking-wide text-white/40">{milestone.date}</div>
                  <p className="mt-1 text-xl font-semibold text-white">{milestone.title}</p>
          ...
          <section className="rounded-3xl border border-white/10 bg-black/40 p-8 text-center">
            <p className="text-sm uppercase tracking-wide text-white/60">What it feels like overall</p>
            <h2 className="mt-4 text-3xl font-semibold text-white">A Web3 super-app that feels alive</h2>
            <p className="mx-auto mt-4 max-w-3xl text-white/70">
              Earnings, leaderboards, AI tweaks, and wallet payouts refresh the moment they happen. That&apos;s the magic of Linera microchains:
              Web2 speed, Web3 fairness, zero middlemen. You watch → you earn. You create → you earn. You advertise → your AI agent works for you.
              Everything updates instantly, transparently, and fairly on-chain.
            </p>
            <div className="mt-8 flex flex-wrap justify-center gap-3 text-xs uppercase tracking-wide text-white/50">
              {['YouTube energy', 'TikTok engagement', 'Google Ads precision', 'DeFi liquidity', 'AI autonomy', 'On-chain trust'].map((tag) => (
          ...
          <section className="grid gap-6 md:grid-cols-3">
            {supportChannels.map((channel) => (
              <div key={channel.label} className="rounded-2xl border border-white/10 bg-white/5 p-5 text-center">
                <p className="text-sm uppercase tracking-wide text-white/50">{channel.label}</p>
